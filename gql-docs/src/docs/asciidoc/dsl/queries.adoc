...
Execution context can be used for sharing values among instrumentations, and data fetchers. A
typical example is when leaving an authenticated user information inside the context. Check
the following example, if there's a user inside the context the data fetcher will return a
welcoming message, otherwise will return `unauthorized`.

[source, groovy]
.context
----
include::{testMain}/gql/dsl//ExecutionSpec.groovy[tags=executionWithContextQuery, indent=0]
----

The way data can be injected when executing a query is with `withContext` method inside `DSL.execute`:

[source, groovy]
.context
----
include::{testMain}/gql/dsl/ExecutionSpec.groovy[tags=executionWithContextExecution, indent=0]
----

If no value has been provided for the context, then a null value is expected.

[sidebar]
.Differences from graphql-java regarding context default value
****
From graphql-java 12 by default when there's no explicit context declared a new GraphQLContext
object is created. In GQL we're still keeping the old behavior, so unless you set explicitly
a context, the context is expected to be null.
****
...
using `DSL.buildMutation` and then use `DSL.execute` to execute the mutation against the
